...
function shortenJSON(data){
    return {summary: data.summary,  
        contact_reason_level1: data.contactReason.level1, 
        contact_reason_level2: data.contactReason.level2,
        contact_reason_level3: data.contactReason.level3 
    }
}

function analyse(query={hasSummary: true}, maxSampleSize=1000){
        ...
        try {
            ...
            if (summaries.length <= maxSampleSize){
                for ( let i = 0; i < summaries.length; i++) {
                    messages.push({role: "user", content: JSON.stringify(summaries[i])})
                }
            }
            else {
                for ( let i = 0; i < maxSampleSize; i++){
                    ...
                    messages.push({role: 'user', content: JSON.stringify(obj)})
                }
            }
            ...
            const result = await client.getChatCompletions(GPT4_MODEL_NAME, messages);
